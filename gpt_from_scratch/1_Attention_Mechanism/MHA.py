...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        assert head_size * num_heads == attention_dim, "attention_dim must be divisible by num_heads"
        
        for _ in range(num_heads):
            self.heads.append(self.SingleHeadAttention(embedding_dim, head_size))

    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        heads_outs = [head(embedded) for head in self.heads]
        out = torch.cat(heads_outs, dim = 2)
